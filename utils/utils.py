...
def pretrain(generator, discriminator, training_loader, gauss_filter, grid_identity, mu, mse_loss, d_loss, optimizer_g, optimizer_d, epoch, n_epochs,
...
def preval(generator, discriminator, validation_loader, gauss_filter, grid_identity, mu, mse_loss, d_loss, epoch, n_epochs, 
           show_every_n_epochs, show_every_n_iters, save_path):
...
def train(generator, training_loader, gauss_filter, grid_identity, mu, dis_loss, ac_loss, verification_loss, dis_loss_weight, ac_loss_weight, 
          ver_loss_weight, optimizer_dis, optimizer_g, optimizer_ac, optimizer_ver, criterion_dis, criterion_ac, criterion_ver, epoch, n_epochs, 
          show_every_n_epochs, show_every_n_iters, save_path):
    ...
    for i, batch in enumerate(training_loader):
        ...
        # Compute DC loss
        dis_loss_value = dis_loss(inputs1, fakes_1)
        ...
        # Put auxiliary classifier loss and verification loss and discriminator loss together
        total_loss = ac_loss_weight * ac_loss_value + ver_loss_weight * log_likelihood_ver_loss + dis_loss_weight * dis_loss_value

        # Append loss values to respective lists
        list_ac_loss.append(ac_loss_value.item())
        list_dis_loss.append(dis_loss_value.item())
        ...
        real_labels = torch.ones(inputs1.size(0), 1).cuda()
        fake_labels = torch.zeros(fakes_1.size(0), 1).cuda()

        real_image = transform_dis(inputs1)
        deformed_image = transform_dis(fakes_1.detach())

        dis_r = dis_loss.dis_model(real_image)
        dis_f = dis_loss.dis_model(deformed_image)

        loss_r = criterion_dis(dis_r, real_labels)
        loss_f = criterion_dis(dis_f, fake_labels)

        total_dis_loss = loss_r + loss_f

        total_dis_loss.backward()
        optimizer_dis.step()
        dis_loss.dis_model.eval()

        # Print information
        print('Epoch [%d/%d], Iteration [%d/%d], Verification Loss (ver_loss): %.4f' % (epoch + 1, n_epochs, i + 1, len(training_loader), ver_loss.item()))

    return [mean(list_ac_loss), mean(list_dis_loss), mean(list_ver_loss), mean(list_log_likelihood_ver_loss), mean(list_total_loss)]


def validate(generator, validation_loader, gauss_filter, grid_identity, mu, dis_loss, ac_loss, verification_loss, dis_loss_weight, 
             ac_loss_weight, ver_loss_weight, epoch, n_epochs, show_every_n_epochs, show_every_n_iters, save_path):
...
def append_losses_to_dict(loss_dict, phase, losses):
    ...
    loss_dict[phase]['dis_loss'].append(losses[1])
    loss_dict[phase]['ver_loss'].append(losses[2])
    loss_dict[phase]['log_likelihood_ver_loss'].append(losses[3])
    loss_dict[phase]['total_loss'].append(losses[4])
    return loss_dict
...
def show_loss_curves(loss_dict, pre_train=True, save_fig=True, show_fig=False, path='./'):
    ...
    if pre_train is True:
        ...
        for phase in ['training', 'validation']:
            ...
            plt.plot(range(1, len(loss_dict[phase]['dis_loss']) + 1), loss_dict[phase]['dis_loss'], label='dis_loss')
            plt.plot(range(1, len(loss_dict[phase]['ver_loss']) + 1), loss_dict[phase]['ver_loss'], label='ver_loss')
            plt.plot(range(1, len(loss_dict[phase]['log_likelihood_ver_loss']) + 1), loss_dict[phase]['log_likelihood_ver_loss'], 
                     label='log_likelihood_ver_loss')
            plt.plot(range(1, len(loss_dict[phase]['total_loss']) + 1), loss_dict[phase]['total_loss'], label='total_loss')
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            if phase == 'training':
