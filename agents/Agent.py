...
from utils.DCLoss import DCLoss
...
class Agent:
    def __init__(self, config):
        ...
        self.dis_loss_weight = self.config['dis_loss_weight']
        ...
        # Define the dicriminator (self.dis_model is already on GPU)
        self.dis_model = Discriminator().cuda()
        self.dis_loss = DCLoss(dis_model=self.dis_model).cuda()
        ...
        self.optimizer_dis = optim.Adam(self.dis_loss.dis_model.parameters(), lr=self.learning_rate)
    ...
    def training_validation(self):
        ...
        for epoch in range(self.start_epoch, self.max_epochs):
            ...
            train_losses = utils.train(self.generator, self.training_loader, self.gauss_filter, self.grid_identity, self.mu, 
                                       self.dis_loss, self.ac_loss, self.verification_loss, self.dis_loss_weight, self.ac_loss_weight, 
                                       self.ver_loss_weight, self.optimizer_dis, self.optimizer_g, self.optimizer_ac, self.optimizer_ver, 
                                       self.criterion_dis, self.criterion_ac, self.criterion_ver, epoch, self.max_epochs, 
                                       self.show_every_n_epochs, self.show_every_n_iterations, self.SAVINGS_PATH)

            # Validate the anonymization model
            val_losses = utils.validate(self.generator, self.validation_loader, self.gauss_filter, self.grid_identity, self.mu, 
                                        self.dis_loss, self.ac_loss, self.verification_loss, self.dis_loss_weight, self.ac_loss_weight, 
            ...
            torch.save(self.dis_loss.dis_model.state_dict(), self.SAVINGS_PATH + 'dis_model_trained_latest.pth')
            ...
            # Save flow field generator that produces the lowest dis_loss
            if val_losses[1] < self.lowest_ac_loss:
                self.lowest_ac_loss = val_losses[1]
                torch.save(self.generator.state_dict(), self.SAVINGS_PATH + 'generator_lowest_dis_loss.pth')
                print('Current generator with lowest dis_loss: epoch ' + str(epoch))

            # Save flow field generator that produces the lowest ver_loss
            if val_losses[2] < self.lowest_ver_loss:
                self.lowest_ver_loss = val_losses[2]
            ...
            if val_losses[4] < self.lowest_total_loss:
                self.lowest_total_loss = val_losses[4]
                ...
                torch.save(self.dis_loss.dis_model.state_dict(), self.SAVINGS_PATH + 'dis_model_trained_lowest_total_loss.pth')
